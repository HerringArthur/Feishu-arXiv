...
from utils import llm_chat, get_llm_model, send_feishu_message, build_reading_result_card
...
def analyze_paper(paper: dict, model: str = None) -> dict:
    """对单篇论文进行决策性阅读分析。"""
    if not model:
        model = get_llm_model()
...
def run_reading(
    ...
    analysis = analyze_paper(paper, model=model)
